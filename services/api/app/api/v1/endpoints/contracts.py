...
async def get_contract_clauses(
    ...
    # If contract not found or wrong user, create sample data
    if not contract or (contract.user_id != user.id):
        ...
        if contracts:
            ...
            analysis = AnalysisResult(
                ...
                top_concerns=["Non-compete restricts job", "No termination notice", "IP belongs to company"],
                top_positives=["30-day payment", "Clear law", "Negotiable"],
                negotiating_power="Weak",
                power_score=45,
                power_label="Favors Counterparty",
                leverage_points=["Limit non-compete", "Add notice", "Share IP"]
            )
            db.add(analysis)
            await db.commit()
