...
ALL_PATTERNS: List[RiskPattern] = [
    ...
    RiskPattern(
        ...
        pattern=_p(r"shall\s+indemnify\s+and\s+hold\s+harmless"),
    ...
    RiskPattern(
        name="all_ip_created",
        pattern=_p(r"all\s+(intellectual\s+property|ip|inventions?|works?|developments?)\s+(created|developed|made|conceived)"),
    ...
    RiskPattern(
        ...
        pattern=_p(r"(shall\s+not|may\s+not|prohibited\s+from).{0,80}(compet|engag|participat).{0,60}(rival|competing|similar)\s+business"),
        category=RiskCategory.NON_COMPETE,
        risk_level=RiskLevel.HIGH,
    ...
    RiskPattern(
        ...
        pattern=_p(r"(worldwide|global|nationwide|statewide).{0,60}non[-\s]?compet"),
        category=RiskCategory.NON_COMPETE,
        risk_level=RiskLevel.HIGH,
