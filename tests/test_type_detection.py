...
class TestDetectContractType:
    ...
    def test_returns_pydantic_model(self):
        ...
        pytest.importorskip("anthropic", reason="requires anthropic SDK")
        from services.ai.app.pipelines.type_detection import detect_contract_type, TypeDetectionResult, ContractType
        import anthropic

        mock_response = MagicMock()
        mock_response.content = [MagicMock(text='{"type": "Employment", "confidence": 0.85, "party_roles": ["employer", "employee"]}')]

        mock_instance = MagicMock()
        mock_instance.messages.create.return_value = mock_response

        result = detect_contract_type("employment contract", anthropic_client=mock_instance)
